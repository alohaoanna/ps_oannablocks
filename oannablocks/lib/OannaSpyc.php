...
if (!function_exists('oanna_spyc_load')) {
  /**
   * Parses YAML to array.
   * @param string $string YAML string.
   * @return array
   */
  function oanna_spyc_load ($string) {
    return OannaSpyc::YAMLLoadString($string);
  }
}

if (!function_exists('oanna_spyc_load_file')) {
  /**
   * Parses YAML to array.
   * @param string $file Path to YAML file.
   * @return array
   */
  function oanna_spyc_load_file ($file) {
    return OannaSpyc::YAMLLoad($file);
  }
}

if (!function_exists('oanna_spyc_dump')) {
  /**
   * Dumps array to YAML.
   * @param array $data Array.
   * @return string
   */
  function oanna_spyc_dump ($data) {
    return OannaSpyc::YAMLDump($data, false, false, true);
  }
}

if (!class_exists('OannaSpyc')) {
...
/**
   ...
   * Usage:
   * <code>
   *   OannaSpyc  = new OannaSpyc;
   *   $array = $OannaSpyc->load($file);
   * </code>
   * or:
   * <code>
   *   $array = OannaSpyc::YAMLLoad($file);
   * </code>
   * or:
   * <code>
   *   $array = oanna_spyc_load_file($file);
   * </code>
   * @package OannaSpyc
   */
class OannaSpyc {
 ...
 */
  ...
  /**
     ...
     *  Usage:
     *  <code>
     *   $array = OannaSpyc::YAMLLoadString("---\n0: hello world\n");
     *   print_r($array);
     *  </code>
     * @access public
     * @return array
...
// Enable use of OannaSpyc from command line
// The syntax is the following: php OannaSpyc.php spyc.yaml

do {
  if (PHP_SAPI != 'cli') break;
  if (empty ($_SERVER['argc']) || $_SERVER['argc'] < 2) break;
  if (empty ($_SERVER['PHP_SELF']) || FALSE === strpos ($_SERVER['PHP_SELF'], 'OannaSpyc.php') ) break;
  $file = $argv[1];
  echo json_encode (oanna_spyc_load_file ($file));
} while (0);
